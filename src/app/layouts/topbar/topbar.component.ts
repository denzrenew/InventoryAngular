...
export class TopbarComponent implements OnInit {
  ...
  ngOnInit() {
    const currentUser : any = this.authFackservice.currentUserValue;
    
    if(this.authService2.getAccessTokenData().role !== 'Administrator') {
      this.userProfileService.getUserInfo().subscribe (
        data=>{
          this.username = data.data.firstName
          if (data.status !== "success") {
            console.log('oops', "Could not retreive userinfo at this time.") 
          }
        }, error => {
